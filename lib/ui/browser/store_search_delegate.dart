...
class StoreSearchDelegate extends SearchDelegate {
  StoreSearchDelegate({required this.list})
      : super(
  ...
  Widget buildSuggestions(BuildContext context) {
    ...
    for (Store? item in list) {
      if (query.trim().isNotEmpty && item!.name!.toLowerCase().contains(query.toLowerCase().trim())) {
        matchQuery.add(item);
      }
    }
        ...
        : ListView.builder(
            itemBuilder: (context, index) {
              ...
              return ListTile(
                ...
                title: RichText(
                  text: TextSpan(
                    children: highlightOccurrences(result.name ?? '', query),
